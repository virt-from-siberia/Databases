SELECT *
FROM EMPLOYEES;

SELECT *
FROM DEPARTMENTS;

DROP TABLE FACULTIES;

CREATE TABLE FACULTIES
(
    ID   NUMBER PRIMARY KEY,
    NAME VARCHAR2(15)
);


CREATE TABLE STUDENTS
(
    ID         NUMBER,
    NAME       VARCHAR2(15),
    COURSE     NUMBER NOT NULL,
    FACULTY_ID INTEGER
);

DROP TABLE STUDENTS;

CREATE TABLE STUDENTS
(
    ID         NUMBER,
    NAME       VARCHAR2(15),
    COURSE     NUMBER NOT NULL,
    FACULTY_ID INTEGER
        CONSTRAINT st_faculty_fk REFERENCES FACULTIES (ID) ON DELETE CASCADE
);

INSERT INTO FACULTIES
VALUES (1, 'CS');

INSERT INTO FACULTIES
VALUES (2, 'Marketing');

COMMIT;

SELECT *
FROM STUDENTS;

SELECT *
FROM FACULTIES;

INSERT INTO STUDENTS
VALUES (1, 'Aleksey', 3, 1);

SELECT *
FROM STUDENTS;

INSERT INTO STUDENTS
VALUES (2, 'Inav', 1, 2);

SELECT *
FROM STUDENTS;

INSERT INTO STUDENTS
VALUES (2, 'Misha', 1, NULL);

SELECT *
FROM STUDENTS;

INSERT INTO STUDENTS
VALUES (1, 'Aleksey', 3, 1);

INSERT INTO STUDENTS
VALUES (2, 'Inav', 1, 2);

SELECT *
FROM STUDENTS;

INSERT INTO STUDENTS
VALUES (2, 'Maxim', 1, NULL);

INSERT INTO STUDENTS
VALUES (2, 'Lena', 1, 1);


DROP TABLE STUDENTS;

SELECT *
FROM STUDENTS;

CREATE TABLE STUDENTS
(
    ID         NUMBER,
    NAME       VARCHAR2(15),
    COURSE     NUMBER NOT NULL,
    FACULTY_ID INTEGER
        CONSTRAINT st_faculty_fk REFERENCES FACULTIES (ID)
);

DROP TABLE STUDENTS;

CREATE TABLE STUDENTS
(
    ID         NUMBER,
    NAME       VARCHAR2(15),
    COURSE     NUMBER NOT NULL,
    FACULTY_ID INTEGER,
    CONSTRAINT fk FOREIGN KEY (FACULTY_ID) REFERENCES FACULTIES (ID)
);

DROP TABLE STUDENTS;

CREATE TABLE STUDENTS
(
    ID         NUMBER,
    NAME       VARCHAR2(15),
    COURSE     NUMBER NOT NULL,
    FACULTY_ID INTEGER
);

ALTER TABLE STUDENTS
    MODIFY (FACULTY_ID CONSTRAINT fk REFERENCES FACULTIES (ID));

DROP TABLE STUDENTS;

CREATE TABLE STUDENTS
(
    ID         NUMBER,
    NAME       VARCHAR2(15),
    COURSE     NUMBER NOT NULL,
    FACULTY_ID INTEGER
);

ALTER TABLE STUDENTS
    MODIFY (FACULTY_ID REFERENCES FACULTIES (ID));

DROP TABLE STUDENTS;

ALTER TABLE STUDENTS
    ADD CONSTRAINT fk FOREIGN KEY (FACULTY_ID) REFERENCES FACULTIES (ID);


SELECT *
FROM STUDENTS;

DROP TABLE STUDENTS;


CREATE TABLE STUDENTS
(
    ID         NUMBER,
    NAME       VARCHAR2(15),
    COURSE     NUMBER NOT NULL,
    FACULTY_ID INTEGER
);

ALTER TABLE STUDENTS
    ADD FOREIGN KEY (FACULTY_ID) REFERENCES FACULTIES (ID);

DROP TABLE STUDENTS;

CREATE TABLE STUDENTS
(
    ID         NUMBER,
    NAME       VARCHAR2(15),
    COURSE     NUMBER NOT NULL,
    FACULTY_ID INTEGER
);

DROP TABLE FACULTIES;

CREATE TABLE FACULTIES
(
    ID   NUMBER PRIMARY KEY,
    NAME VARCHAR2(15)
);




























